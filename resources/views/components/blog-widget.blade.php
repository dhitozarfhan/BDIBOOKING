    ...
    <div class="mb-8">
        ...
        <div class="space-y-2">
            @foreach ($category as $data)
                <a href="{{  route('blog.index', ['categoryId' => $data->category_id, 'categorySlug' => Str::slug($data->id_name)]) }}"
                    class="block bg-white border border-gray-300 p-3 rounded-lg hover:border-blue-500">
                    <div class="flex items-center justify-between">
                        <h6 class="text-sm font-semibold">
                            <i class="far fa-folder-open"></i>
                            {{ $data->id_name }}
                            <small class="text-gray-500">({{ $data->blog_count }})</small>
                        </h6>
...
<div x-data="{ activeTab: 'recent' }" class="space-y-4">
    ...
    <div>
        <div x-show="activeTab === 'recent'" x-cloak>
            <h3 class="mb-4 text-xl font-bold">Blog Terkini</h3>
        ...
        <div x-show="activeTab === 'popular'" x-cloak>
            <h3 class="mb-4 text-xl font-bold">Blog Populer</h3>
