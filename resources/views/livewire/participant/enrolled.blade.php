<div class="min-h-screen bg-base-200/30">
    <div class="flex container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex-1 min-w-0 p-8 lg:p-10">
            <div class="mb-8">
                <h1 class="text-2xl font-bold text-base-content tracking-tight">Layanan Pelatihan & Properti</h1>
                <p class="text-sm text-base-content/50 mt-1">Layanan Diklat & Properti yang sedang Anda ikuti atau menunggu persetujuan.</p>
            </div>
            ...
            <div class="space-y-4">
                @forelse($bookings as $booking)
                    @php
                        ...
                        $detailUrl = $isTraining
                            ? route('pnbp.detail', ['id_diklat' => $booking->bookable->id, 'slug' => Str::slug($booking->bookable->title)])
                            : route('pnbp.properti.detail', ['id' => $booking->bookable->id, 'slug' => Str::slug($booking->bookable->name)]);
                    @endphp
                            ...
                            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex flex-wrap items-center gap-4 text-sm text-base-content/50">
                                        ...
                                        @endif
                                        @if($booking->bookable->price ?? false)
                                            <span class="inline-flex items-center gap-1.5">
                                                <i class="bi bi-cash-stack text-base-content/30"></i>
                                                Rp {{ number_format($booking->bookable->price, 0, ',', '.') }}
                                            </span>
                                        @endif
                                    </div>
                                </div>
                                <div class="shrink-0">
                                    <a href="{{ $detailUrl }}" class="btn btn-sm btn-ghost text-indigo-600 hover:bg-indigo-50 rounded-xl gap-1.5">
                ...
                @empty
                    <div class="bg-base-100 rounded-2xl border border-base-200/80 px-12 pt-10 pb-12 text-center">
                        ...
                        <p class="text-sm text-base-content/40 font-medium">Belum ada layanan yang sedang diikuti.</p>
