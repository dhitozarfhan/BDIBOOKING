            ...
            <div class="p-4">
                <nav class="space-y-2">
                    <a href="{{ route('information.answer') }}"
                       class="flex items-center justify-between px-4 py-3 text-sm font-medium rounded-lg {{ !$status ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                    ...
                    <a href="{{ route('information.answer', ['status' => 'baru']) }}"
                       class="flex items-center justify-between px-4 py-3 text-sm font-medium rounded-lg {{ $status == 'baru' ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('information.answer', ['status' => 'diproses']) }}"
                       class="flex items-center justify-between px-4 py-3 text-sm font-medium rounded-lg {{ $status == 'diproses' ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('information.answer', ['status' => 'disposisi']) }}"
                       class="flex items-center justify-between px-4 py-3 text-sm font-medium rounded-lg {{ $status == 'disposisi' ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('information.answer', ['status' => 'selesai']) }}"
                       class="flex items-center justify-between px-4 py-3 text-sm font-medium rounded-lg {{ $status == 'selesai' ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-xs font-semibold tracking-wider text-left text-blue-600 uppercase">{{ __('Kd Reg') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold tracking-wider text-left text-blue-600 uppercase">{{ __('Penanya') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold tracking-wider text-left text-blue-600 uppercase">{{ __('Pertanyaan') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold tracking-wider text-left text-blue-600 uppercase">{{ __('Kategori') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold tracking-wider text-left text-blue-600 uppercase">{{ __('Status') }}</th>
                            </tr>
                        </thead>
                                ...
                                @foreach ($items as $item)
                                    ...
                                    <tr onclick="window.location='{{ route('information.answer.detail', ['registration_code' => $item->registration_code]) }}'" class="hover:bg-gray-50 transition-colors cursor-pointer">
                                            ...
                                            <div class="max-w-lg">
                                                @if ($item instanceof \App\Models\InformationRequest)
                                                    ...
                                                    @if (strlen($item->report_title) > 100)
                                                        <span class="text-blue-600 hover:underline">{{ __('Lihat jawaban') }}</span>
                                                    @endif
                                        ...
                                        <td class="px-6 py-4 text-sm">
                                            @if ($item instanceof \App\Models\InformationRequest)
                                                <span class="text-gray-900">{{ __('Permohonan Informasi') }}</span>
                                            @elseif ($item instanceof \App\Models\Question)
                                                <span class="text-gray-900">{{ __('Pengaduan Masyarakat') }}</span>
                                            @endif
                                        </td>
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-16 text-sm text-center text-gray-500">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-gray-600">{{ __('Tidak ada data untuk ditampilkan.') }}</p>
                                        </div>
                                    </td>
                                </tr>
                            @endif
